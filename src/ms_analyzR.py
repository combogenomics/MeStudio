...
def bed_builder(msgff3, annogff3):
    ...
    motif_name = os.path.basename(path_anno.split("_")[0])
    feature_name = os.path.basename(path_anno.split("_")[1])
    ## Define the dataframes
    anno_df = pd.read_table(path_anno, header=None)
    anno_df.columns = ["seqid", "source", "type", "start", "end", "score", "strand", "phase", "attribute"]
    # - - - - - - - - - - - - - - #
    ms_df = pd.read_table(path_ms, header=None)
    ms_df.columns = ["seqid", "source", "type", "start", "end", "score", "strand", "phase", "attribute"]
    ...
    bed_df.columns = ['seqid', 'start', 'end','gene-id', 'gene-anno']  # rename 'em all
    ## Add a 5th column 'score' which takes the number of methylations x gene (attribute)
    ms_df = ms_df.drop_duplicates(subset='attribute').assign(score=ms_df.groupby('attribute')['attribute'].transform('count'))
    ms_df = ms_df.reset_index(drop=True)

    match_list = [] # initi a list for storing the results
    for gene in ms_df["attribute"]:
        if not bed_df.loc[(bed_df == gene).any(1), 'gene-anno'].empty:
            match = str(bed_df.loc[(bed_df == gene).any(1), 'gene-anno'].reset_index(drop=True).to_list()[0])
...
def roary_reader(rr_csv):
    ...
    roary_df = pd.read_csv(path_infile, index_col=False, dtype='unicode', on_bad_lines='skip', sep=",")

    core_value = roary_df["No. isolates"].max() # get the corresponding value for "core" genes
    core_df = roary_df[roary_df["No. isolates"].str.contains(core_value)] # get the core genes dataframe
    cloud_df = roary_df[~roary_df["No. isolates"].str.contains(core_value)] # get the dispensable genes dataframe
...
def roary_miner(rr_csv, id):
    ...
    match = str(roary_df.loc[(roary_df == id).any(1), 'Annotation'].reset_index(drop = True).to_list()[0])
    core_match = str(roary_df.loc[(roary_df == id).any(1), 'No. isolates'].reset_index(drop = True).to_list()[0])
...
def ms_gff(gff3):
    ...
    name_motif = os.path.basename(path_infile.split("_")[0])
    name_feature = os.path.basename(path_infile.split("_")[1])
    with open(path_infile, 'r') as g:
        ## Init the dataframe
        gff_df = pd.read_table(g, header=None)
        gff_df.columns=["seqid", "source", "type", "start", "end", "score", "strand", "phase", "attribute"]
        ## Operate to the dataframe:
        gff_cdf = gff_df[gff_df['score'] != 0] ## Filter out '0' values from 'score'
        ## Get some numbers:
        n_motifs = gff_df.shape[0] ## total number of found motifs
        n_memotifs = gff_cdf.shape[0] ## total number of methylated found motifs
        ## Check -s flag:
        if strict == True:
            ## Occurrence check: if no methylations have been found, stop the analysis.
            if n_memotifs == 0:
                sys.exit(f"Sorry, the analysis has been stopped: you have not enough methylations for {name_feature} ")
        ...
        occurrences_count= gff_cdf['attribute'].value_counts().to_frame(name='occurrences') ## dataframe with N* methylations x attribute
        hmo = occurrences_count['occurrences'].max() ## H-ighest M-ethylation O-ccurence
        hmg = gff_cdf['attribute'].value_counts().iloc[[0]].keys()[0] ## H-ighest M-ethylated G-ene
        
        ## Printing some info to the STDOUT
        if os.path.basename(path_infile).endswith("_CDS.gff"):
            stdout_logger(name_motif, "CDS", me_ratio, hmg, hmo, n_motifs, n_memotifs)

        if os.path.basename(path_infile).endswith("_nCDS.gff"):
            stdout_logger(name_motif, "nCDS", me_ratio, hmg, hmo, n_motifs, n_memotifs)

        if os.path.basename(path_infile).endswith("_tIG.gff") or os.path.basename(path_infile).endswith("_intergenic.gff"):
            stdout_logger(name_motif, "tIG", me_ratio, hmg, hmo, n_motifs, n_memotifs)

        if os.path.basename(path_infile).endswith("_US.gff") or os.path.basename(path_infile).endswith("_upstream.gff"):
            stdout_logger(name_motif, "US", me_ratio, hmg, hmo, n_motifs, n_memotifs)


# Arg-parser:
def ms_analyzR() -> tuple:
    ap = argparse.ArgumentParser(description='Usage:')
    ap.add_argument("-o", "--outputdir", help="path to your output files")
    ap.add_argument("-CDS", "--coding", help="CDS.gff file")
    ap.add_argument("-nCDS", "--noncoding", help="nCDS.gff file")
    ap.add_argument("-tIG", "--intergenic", help="tIG.gff file")
    ap.add_argument("-US", "--upstream", help="US.gff file")
    ...
    ap.add_argument("-s", "--strict", help="If used, this flag stops the analysis when 0 methylations are found in the GFF3.", action="store_true", default=False)
    ap.add_argument("-r", "--roary", help="path to 'gene_presence_abscence.csv' file produced by Roary", action="store_true")
    ap.add_argument("-q", "--quiet", help="Running ms analyzR quietly (no stdout)",action="store_true", default=False)
...
if chrom == True:
    print("[OK] Splitting your files to replicon level")
